    ...
    async function initializeWithEmotionData() {
        ...
        if (conversationChoice) {
            if (conversationChoice === 'yes') {
                ...
                if (!userEmotion) {
                    addMessageToChat('ai', '抱歉，加载您的情绪数据时出错。请重试。');
                    return;
                }
                try {
                    ...
                    window.location.href = 'audio-zh.html';
                } catch (error) {
                    addMessageToChat('ai', '抱歉，启动冥想时出错。请重试。');
                }
                return;
            }
        }
...
Now let's move into a guided meditation practice.`;
    ...
    function startHotCrossBunConversation() {
        ...
        addMessageToChat('system', '正在启动引导对话...');
        ...
        .then(data => {
            ...
            if (data.response) {
                ...
                addMessageToChat('ai', '抱歉，从服务收到了意外的响应。');
                enableInput();
            }
        })
        .catch(error => {
    ...
    async function sendHotCrossBunMessageToAPI(message, isSystem = false, isUncertain = false) {
        ...
        try {
            ...
            if (data.response) {
                ...
                if (data.response.includes('引导冥想练习') || data.response.toLowerCase().includes('guided meditation practice') || 
                    data.response.includes('进入引导冥想') || data.response.toLowerCase().includes('move into meditation') ||
                    data.response.includes('冥想练习') || data.response.toLowerCase().includes('meditation session')) {
                    ...
                    setTimeout(() => {
                        addMessageToChat('system', '正在生成您的个性化冥想引导...');
                        ...
                        setTimeout(async () => {
                            try {
                                ...
                                addMessageToChat('ai', '抱歉，生成您的冥想时出错。请重试。');
                            }
                        }, 2000);
                    }, 2000);
                    return;
                ...
                if (hcbExchangeCount >= 8 && !data.response.toLowerCase().includes('guided meditation')) {
                    setTimeout(() => {
                        ...
                        buttonDiv.innerHTML = `
                            <button id="continueConversation">继续对话</button>
                            <button id="startMeditation">开始冥想</button>
                        `;
    ...
    async function processLineB() {
        ...
        try {
            ...
            buttonDiv.innerHTML = `
                <button id="startMeditation">开始冥想</button>
                <button id="continueChatting">继续聊天</button>
            `;
            chatWindow.appendChild(buttonDiv);
    ...
    function initEmotionInput() {
        chatWindow.innerHTML = `
            <div class="welcome-message">
                ...
                <button id="submitEmotion">继续</button>
            </div>
        `;
    ...
    function offerHotCrossBun() {
        ...
        chatWindow.innerHTML = `
            <div class="message ai">
                <div class="bubble">
                    感谢您的分享。您想通过引导对话更深入地探索这种感觉吗？
                </div>
            </div>
            <div class="choice-buttons">
                <button id="acceptHCB">是的，我想</button>
                <button id="rejectHCB">不，直接继续</button>
            </div>
        `;
